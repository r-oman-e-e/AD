...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton("excuseButton", "Tu veux oublier cette personne ?")
    ),
...
server <- function(input, output, session) {
  ...
  # Ajouter une excuse pour boire
  observeEvent(input$excuseButton, {
    output$excuseMessage <- renderUI({
      tagList(
        tags$h3("Tu veux oublier cette personne ?"),
        actionButton("yesButton", "Oui", style = "background-color: green; color: white;"),
        actionButton("noButton", "Non", style = "background-color: orange; color: white;")
      )
    })
  })
  ...
  observeEvent(input$yesButton, {
    output$excuseMessage <- renderUI({
      tags$p("Une excuse parfaite pour boire ! Santé !")
    })
  })
  
  observeEvent(input$noButton, {
    output$excuseMessage <- renderUI({
      tags$p("Pas besoin d'excuse pour boire ! Santé !")
    })
  })
}
