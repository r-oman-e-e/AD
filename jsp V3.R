...
# Obtenir une liste unique de catégories
unique_categories <- unique(all_cocktails$category)
unique_categories <- unique_categories[!is.na(unique_categories)] # Retirer les valeurs NA
...
# Fonction mise à jour pour trouver des cocktails par catégorie
find_by_category <- function(selected_categories) {
  matching_cocktails <- all_cocktails %>%
    filter(category %in% selected_categories)
  
  if (nrow(matching_cocktails) == 0) {
    return("Aucun cocktail trouvé pour cette ou ces catégories.")
  } else {
    return(matching_cocktails)
  }
}
...
main_function <- function() {
  ...
  if (choice == "1") {
    selected_indices <- menu(unique_categories, title = "Sélectionnez une ou plusieurs catégories (appuyez sur 0 pour terminer) :", graphics = FALSE)
    if (selected_indices == 0) {
      cat("Aucune catégorie sélectionnée. Veuillez redémarrer.\n")
      return()
    }
    ...
    if (is.character(matching_cocktails)) {
      ...
      explore_cocktails(matching_cocktails)
    }
  } else if (choice == "2") {
...
# Nouvelle fonction pour explorer les cocktails trouvés
explore_cocktails <- function(matching_cocktails) {
  cocktail_list <- matching_cocktails$cocktail_name
  repeat {
    selected_cocktail <- menu(cocktail_list, title = "Choisissez un cocktail pour voir les détails (appuyez sur 0 pour quitter) :")
    if (selected_cocktail == 0) {
      cat("Merci d'avoir utilisé notre application ! À bientôt.\n")
      break
    }
    show_cocktail_details(matching_cocktails, cocktail_list[selected_cocktail])
    another <- readline(prompt = "Souhaitez-vous voir un autre cocktail ? (oui/non) : ")
    if (tolower(another) != "oui") {
      cat("Merci d'avoir utilisé notre application ! À bientôt.\n")
      break
    }
  }
